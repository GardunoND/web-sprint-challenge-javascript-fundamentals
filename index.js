...
/* the nested function can access the variable internal due to it being nested in myFunction where the variable is located.
this also shows a closure, where an inner function reaches out (outer scope) and accesses information from the outer scope.
Note that closures can only reach out, not in.*/
...
const zooAnimals = [
  ...
  */
  ...
  function lowerCaseNames(zooAnimals){
    const newArr = zooAnimals.map((key) => key.animal_name.toLowerCase());
    return newArr;
  }
  ...
  */
  ...
  function lowPopulationAnimals(zooAnimals){
    const newArr = zooAnimals.filter((key) => {
      return key.population < 5;
    });
    return newArr;
  }
  ...
  */
  ...
  function USApop(zooAnimals){
    const totPops = zooAnimals.reduce((acc, key) => {
      return acc + key.population;
    }, 0);
    return totPops;
  }
